$ ->
  ...
  $('#play-button').on 'click', ->
    ...
    tl.from('#title', 1, {}, 0)
    tl.staggerFrom(splitText('#me-monster-1'), 0.5, {alpha:0}, 0.3, 10.178)
    tl.staggerFrom(splitText('#me-monster-2'), 0.5, {alpha:0}, 0.2, 14.383)

    tl.from('#me-1', 0.5, {alpha:0}, 17.199)
    tl.from('#me-2', 0.5, {alpha:0}, 17.199)
    tl.from('#me-3', 0.5, {alpha:0}, 18.307)
    tl.from('#me-4', 0.5, {alpha:0}, 20.163)
    tl.from('#me-5', 0.5, {alpha:0}, 21.206)
    tl.from('#me-6', 0.5, {alpha:0}, 23.160)
    tl.from('#me-7', 0.5, {alpha:0}, 24.958)
    ...
    tl.staggerFrom(splitText('#nothing'), 0.5, {alpha:0}, 0.3, 39.371)
    tl.from('#waste-1', 0.5, {alpha:0}, 42.079)
    tl.staggerFrom(splitText('#waste-2'), 0.5, {alpha:0}, 0.2, 43.606)
    tl.staggerFrom(splitText('#waste-3'), 0.5, {alpha:0}, 0.3, 48.941)
    tl.staggerFrom(splitText('#waste-4'), 0.5, {alpha:0}, 0.3, 51.428)
    tl.staggerFrom(splitText('#waste-5'), 0.5, {alpha:0}, 0.3, 54.7)
    ...
    tl.staggerFrom(splitText('#barrage-1'), 0.5, {alpha:0}, 0.2, 122.512)
    tl.staggerFrom(splitText('#barrage-2'), 0.5, {alpha:0}, 0.2, 139.446)
    tl.staggerFrom(splitText('#barrage-3'), 0.5, {alpha:0}, 0.2, 140.492)
    tl.staggerFrom(splitText('#barrage-4'), 0.5, {alpha:0}, 0.2, 140.977)
    tl.staggerFrom(splitText('#barrage-5'), 0.5, {alpha:0}, 0.2, 141.675)
    tl.staggerFrom(splitText('#barrage-6'), 0.5, {alpha:0}, 0.2, 142.650)
    tl.staggerFrom(splitText('#barrage-7'), 0.5, {alpha:0}, 0.2, 143.466)
    tl.staggerFrom(splitText('#moon-1'), 0.5, {alpha:0}, 0.3, 150.258)

    tl.to('#moon-token', 1, {rotation:45}, "+0.5")

    tl.staggerFrom(splitText('#moon-2'), 0.5, {alpha:0}, 0.2, 159.755)
    tl.staggerFrom(splitText('#moon-3'), 0.5, {alpha:0}, 0.3, 163.274)
    tl.staggerFrom(splitText('#moon-4'), 0.5, {alpha:0}, 0.3, 167.094)
    tl.staggerFrom(splitText('#moon-5'), 0.5, {alpha:0}, 0.3, 173.781)
    tl.staggerFrom(splitText('#moon-6'), 0.5, {alpha:0}, 0.3, 177.346)
